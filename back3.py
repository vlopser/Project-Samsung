    ...
    def generate():
        while True:
            ...
            if results.pose_landmarks:
                ...
                # Draw a semi-circle on the opposite side of the elbow
                center = tuple(map(int, elbow))
                axes = (100, 100)  # Semi-circle size
                angle_start = 180  # Start at 180 degrees to draw on the opposite side
                angle_end = angle_start + int(smaller_angle)  # End at the calculated angle
                color = (0, 255, 0)  # Green color

                cv2.ellipse(frame, center, axes, 0, angle_start, angle_end, color, 2)

                # Display the angle on the frame
                cv2.putText(frame, f'{int(angle)}',
                            tuple(map(int, elbow)),
                            cv2.FONT_HERSHEY_SIMPLEX, 2.5, (0, 255, 0), 2, cv2.LINE_AA)
